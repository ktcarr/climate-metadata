...
'''
Script to download pressure-level ERA5 data from copernicus server. Server only allows one month of data to be downloaded at a time, so script loops through months requested.
...
LEVELS: pressure levels (hPa) at which to download the data
...
SAVE_PATH = '/vortex/clidex/data/reanalysis/era5/pressure-levels'

for year in YEARS:
    if year=='2021':
        MONTHS = [f'{i:02d}' for i in np.arange(5,0,-1)]
    else:
